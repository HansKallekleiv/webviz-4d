...
from webviz_4d._datainput._surface import make_surface_layer, load_surface_from_blob
...
from webviz_4d._interfaces import SumoConnection
...
from ._surface_metadata_processing import update_selections


class SurfaceViewer4DSumo(WebvizPluginABC):
    """### SurfaceViewer4D """
    ...
    def __init__(
        ...
        self.sumo = SumoConnection(env=sumo_env)
        self.ens_id = self.sumo.ensembles[27]
        self.surface_metadata = self.sumo.get_surface_attributes_in_ensemble(
            self.ens_id
        )
        ...
        self.selection_list = update_selections(
            self.selection_list, self.surface_metadata
        )
    ...
    def get_real_runpath(self, data, ensemble, real, map_type):
        ...
        print(list(self.surface_metadata["fmu_id.realization"].unique()))
        selected_metadata = self.surface_metadata[
            self.surface_metadata["fmu_id.realization"] == real
        ]
        print(selected_metadata)
        selected_metadata = selected_metadata[
            (selected_metadata["fmu_id.ensemble"] == ensemble)
        ]
        print(selected_metadata)
        selected_metadata = selected_metadata[
            (selected_metadata["map_type"] == map_type)
        ]
        print("maptype", selected_metadata)
        print(f"{time1[0:4]}{time1[5:7]}{time1[8:10]}")
        print(time1)
        selected_metadata = selected_metadata[
            (
                selected_metadata["data.time.t1"]
                == f"{time1[0:4]}{time1[5:7]}{time1[8:10]}"
            )
        ]
        print(selected_metadata)
        selected_metadata = selected_metadata[
            (
                selected_metadata["data.time.t2"]
                == f"{time2[0:4]}{time2[5:7]}{time2[8:10]}"
            )
        ]
        print(selected_metadata)
        selected_metadata = selected_metadata[(selected_metadata["data.name"] == name)]
        print(selected_metadata["data.content"])
        selected_metadata = selected_metadata[
            (selected_metadata["data.content"] == attribute)
        ]
        print(selected_metadata)
    ...
    def make_map(self, data, ensemble, real, attribute_settings, map_idx):
        ...
        if True:
            surface = load_surface_from_blob(self.sumo.api, surface_file)
